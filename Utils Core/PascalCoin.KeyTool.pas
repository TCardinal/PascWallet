...
{$IFDEF UNITTEST}
    function DoECDSASign(const APrivateKey: IECPrivateKeyParameters;
      const AMessage: TBytes; const K: string = '')
      : TCryptoLibGenericArray<TBigInteger>;
{$ELSE}
    function DoECDSASign(const APrivateKey: IECPrivateKeyParameters;
      const AMessage: TBytes): TCryptoLibGenericArray<TBigInteger>;
{$ENDIF UNITTEST}
    function DoECDSAVerify(const APublicKey: IECPublicKeyParameters;
      const AMessage: TBytes;
      const ASig: TCryptoLibGenericArray<TBigInteger>): boolean;
    function RetrieveKeyTypeInt32FromByteArray(const AInput: TBytes;
      AOffset: Int32): Int32; inline;
    function GetPublicKeyType(const APascalCoinBase58PublicKey: string)
      : TKeyType;
    function GetPrivateKeyType(const AEncryptedPascalCoinPrivateKey,
      APassword: string): TKeyType;
    function ValidatePascalCoinBase58PublicKeyChecksum
      (const APascalCoinBase58PublicKey: string): boolean;
    function ValidatePublicKeyHeader(AKeyType: TKeyType; const AInput: TBytes;
      out AErrorMessage: string): boolean;
    function ValidatePrivateKeyHeader(AKeyType: TKeyType; const AInput: TBytes;
      out AErrorMessage: string): boolean;
    ...
    function ComputeECIESPascalCoinEncrypt(const APublicKey
      : IECPublicKeyParameters; const APayloadToEncrypt: TBytes)
      : TBytes; inline;
    function ComputeECIESPascalCoinDecrypt(const APrivateKey
      : IECPrivateKeyParameters; const APayloadToDecrypt: TBytes;
      out ADecryptedPayload: TBytes): boolean; inline;
    ...
    function ExtractPrivateKeyFromDecryptedPascalCoinPrivateKey
      (const DecryptedPascalCoinPrivateKey: TBytes): TBytes; inline;
    function ExtractAffineXFromPascalCoinPublicKey(const APascalCoinPublicKey
      : string; out AAffineXBytes: TBytes): boolean; inline;
    function ExtractAffineYFromPascalCoinPublicKey(const APascalCoinPublicKey
      : string; out AAffineYBytes: TBytes): boolean; inline;
    function GetCurveFromKeyType(AKeyType: TKeyType): IX9ECParameters; inline;
    function EVP_GetSalt(): TBytes; inline;
    function EVP_GetKeyIV(const APasswordBytes, ASaltBytes: TBytes;
      out AKeyBytes, AIVBytes: TBytes): boolean;
    function ComputeAES256_CBC_PKCS7PADDING_PascalCoinEncrypt
      (const APlainTextBytes, APasswordBytes: TBytes): TBytes;
    function ComputeAES256_CBC_PKCS7PADDING_PascalCoinDecrypt
      (const ACipherTextBytes, APasswordBytes: TBytes;
      out APlainText: TBytes): boolean;
    function GetPrivateKeyPrefix(AKeyType: TKeyType; const AInput: TBytes)
      : TBytes; inline;
    function GetAffineXPrefix(AKeyType: TKeyType; const AInput: TBytes)
      : TBytes; inline;
    ...
    function IsValidKeyTypeMatch(AValue: Int32; AKeyType: TKeyType)
      : boolean; inline;

    function GetDomainParams(const AKeyType: TKeyType): IECDomainParameters;
    function GetCurveParams(const AKeyType: TKeyType): IX9ECParameters;
  ...
  protected
    ...
    function GetPascalCoinPublicKeyAsBase58(const AHexPublicKey
      : string): String;

    function RecreatePrivateKeyFromByteArray(AKeyType: TKeyType;
      const APrivateKey: TBytes): IECPrivateKeyParameters;

    function RecreatePrivateKeyFromHexStr(AKeyType: TKeyType;
      const APrivateKey: string): IECPrivateKeyParameters;
...
{$ENDIF UNITTEST}
  ...
  public
    ...
    procedure GenerateKeyPairAndLog(AKeyType: TKeyType; const APassword: string;
      var Logger: TStringList);
    procedure TestEncryptPascalCoinPrivateKey(AKeyType: TKeyType;
      const APrivateKeyToEncrypt, APassword: string; var Logger: TStringList);
    procedure TestDecryptPascalCoinPrivateKey
      (const AEncryptedPascalCoinPrivateKey, APassword: string;
      var Logger: TStringList);
    procedure TestEncryptPascalCoinECIESPayload(AKeyType: TKeyType;
      const APascalCoinBase58PublicKey, APayloadToEncrypt: string;
      var Logger: TStringList);
    procedure TestDecryptPascalCoinECIESPayload(AKeyType: TKeyType;
      const AEncryptedPascalCoinPrivateKey, APrivateKeyPassword,
      APayloadToDecrypt: string; var Logger: TStringList);
    procedure TestEncryptPascalCoinAESPayload(const APassword, APayloadToEncrypt
      : string; var Logger: TStringList);
    procedure TestDecryptPascalCoinAESPayload(const APassword, APayloadToDecrypt
      : string; var Logger: TStringList);
    procedure Generate_Recreate_Sign_Verify_ECDSA_Stress_Test
      (AKeyType: TKeyType; const AMessage: string; AIterationCount: Int32;
      var Logger: TStringList);
  end;

implementation

uses PascalCoin.Helpers, System.Rtti, clpFixedSecureRandom;
...
function TPascalCoinKeyTools.ComputeSHA2_256_ToBytes(const AInput
  : TBytes): TBytes;
begin
  Result := TDigestUtilities.CalculateDigest('SHA-256', AInput);
end;

function TPascalCoinKeyTools.ComputeBase16EncodeUpper
  (const AInput: TBytes): string;
begin
...
function TPascalCoinKeyTools.GetCurveFromKeyType(AKeyType: TKeyType)
  : IX9ECParameters;
var
  CurveName: string;
begin
...
function TPascalCoinKeyTools.GetCurveParams(const AKeyType: TKeyType)
  : IX9ECParameters;
begin
  Result := TCustomNamedCurves.GetByName
    (TRttiEnumerationType.GetName<TKeyType>(AKeyType));
end;

function TPascalCoinKeyTools.GetDomainParams(const AKeyType: TKeyType)
  : IECDomainParameters;
var
  lCurve: IX9ECParameters;
begin
  lCurve := GetCurveParams(AKeyType);
  Result := TECDomainParameters.Create(lCurve.Curve, lCurve.G, lCurve.N,
...
begin
  case AKeyType of
    ...
    raise EArgumentOutOfRangeException.CreateResFmt(@SInvalidKeyTypeSelected,
      [Ord(AKeyType)]);
  end;

end;

function TPascalCoinKeyTools.IsValidKeyTypeMatch(AValue: Int32;
  AKeyType: TKeyType): boolean;
var
  LKeyType: TKeyType;
begin
  try
...
function TPascalCoinKeyTools.UInt32ToLittleEndianByteArrayTwoBytes
  (AValue: UInt32): TBytes;
begin
  Result := System.Copy(TConverters.ReadUInt32AsBytesLE(AValue), 0, 2);
end;

function TPascalCoinKeyTools.TwoBytesByteArrayToDec(const AValue
  : TBytes): Int32;
begin
  Result := StrToInt('$' + ComputeBase16EncodeUpper(AValue));
end;

function TPascalCoinKeyTools.GetAffineXPrefix(AKeyType: TKeyType;
  const AInput: TBytes): TBytes;
begin
  Result := TArrayUtils.Concatenate(UInt32ToLittleEndianByteArrayTwoBytes
    (UInt32(GetKeyTypeNumericValue(AKeyType))),
...
function TPascalCoinKeyTools.GetPrivateKeyPrefix(AKeyType: TKeyType;
  const AInput: TBytes): TBytes;
begin
  Result := TArrayUtils.Concatenate(UInt32ToLittleEndianByteArrayTwoBytes
    (UInt32(GetKeyTypeNumericValue(AKeyType))),
...
function TPascalCoinKeyTools.DoECDSASign(const APrivateKey
  : IECPrivateKeyParameters; const AMessage: TBytes; const K: string = '')
  : TCryptoLibGenericArray<TBigInteger>;
{$ELSE}

function TPascalCoinKeyTools.DoECDSASign(const APrivateKey
  : IECPrivateKeyParameters; const AMessage: TBytes)
  : TCryptoLibGenericArray<TBigInteger>;
{$ENDIF UNITTEST}
var
  Signer: IECDsaSigner;
...
function TPascalCoinKeyTools.DoECDSAVerify(const APublicKey
  : IECPublicKeyParameters; const AMessage: TBytes;
  const ASig: TCryptoLibGenericArray<TBigInteger>): boolean;
var
  Signer: IECDsaSigner;
begin
...
function TPascalCoinKeyTools.RetrieveKeyTypeInt32FromByteArray
  (const AInput: TBytes; AOffset: Int32): Int32;
var
  ReversedSlice: TBytes;
begin
  System.SetLength(ReversedSlice, 2);
  TBits.ReverseByteArray(System.Copy(AInput, AOffset, 2), ReversedSlice,
    Length(ReversedSlice) * System.SizeOf(byte));
  Result := TwoBytesByteArrayToDec(ReversedSlice);
end;

function TPascalCoinKeyTools.SignOperation(const APrivateKey: TBytes;
  const AKeyType: TKeyType; const AMessage: string;
  var ASignature: TECDSA_Signature): boolean;
var
...
begin
  ...
  ASignature.R := THex.Encode(LSignerResult[0].ToByteArrayUnsigned);
  ASignature.RLen := Length(LSignerResult[0].ToByteArrayUnsigned);
  ASignature.S := THex.Encode(LSignerResult[1].ToByteArrayUnsigned);
  ASignature.SLen := Length(LSignerResult[1].ToByteArrayUnsigned);
  Result := True;
...
function TPascalCoinKeyTools.SignOperation(const APrivateKey: string;
  const AKeyType: TKeyType; const AMessage: string;
  var ASignature: TECDSA_Signature; const K: string = ''): boolean;
{$ELSE}
...
function TPascalCoinKeyTools.GetPublicKeyType(const APascalCoinBase58PublicKey
  : string): TKeyType;
var
  LPascalCoinBase58DecodedPublicKey: TBytes;
begin
  LPascalCoinBase58DecodedPublicKey :=
    ComputeBase58Decode(APascalCoinBase58PublicKey);
  Result := RetrieveKeyType(RetrieveKeyTypeInt32FromByteArray
    (LPascalCoinBase58DecodedPublicKey, 1));
end;

function TPascalCoinKeyTools.GetPrivateKeyType
  (const AEncryptedPascalCoinPrivateKey, APassword: string): TKeyType;
var
  LPascalCoinPrivateKey: TBytes;
  DecryptedSuccessfully: boolean;
begin
  Result := Default (TKeyType);
  DecryptedSuccessfully := DecryptPascalCoinPrivateKey
    (AEncryptedPascalCoinPrivateKey, APassword, LPascalCoinPrivateKey);
  if DecryptedSuccessfully then
    Result := RetrieveKeyType(RetrieveKeyTypeInt32FromByteArray
      (LPascalCoinPrivateKey, 0));
end;

function TPascalCoinKeyTools.ValidatePascalCoinBase58PublicKeyChecksum
  (const APascalCoinBase58PublicKey: string): boolean;
var
  Chopped, DecodedPascalCoinBase58Key, OriginalChecksum,
    CalculatedChecksum: TBytes;
...
begin
  ...
  CalculatedChecksum := System.Copy(DecodedPascalCoinBase58Key,
    DecodedPascalCoinBase58KeyLength - 4, DecodedPascalCoinBase58KeyLength - 1);
  Chopped := System.Copy(DecodedPascalCoinBase58Key, 1,
    DecodedPascalCoinBase58KeyLength - 5);
...
function TPascalCoinKeyTools.ValidatePublicKeyHeader(AKeyType: TKeyType;
  const AInput: TBytes; out AErrorMessage: string): boolean;
var
...
begin
  ...
  case ExtractedValue of
    714, 715, 716, 729:
      ...
      begin
        AErrorMessage :=
          'PascalCoin Public Key Header Does Not Match Selected Key Type.';
        Exit;
      end
  else
    begin
...
function TPascalCoinKeyTools.ValidatePrivateKeyHeader(AKeyType: TKeyType;
  const AInput: TBytes; out AErrorMessage: string): boolean;
var
  ExtractedValue: Int32;
begin
  ...
  case ExtractedValue of
    714, 715, 716, 729:
      ...
      begin
        AErrorMessage :=
          'PascalCoin Private Key Header Does Not Match Selected Key Type.';
        Exit;
      end
  else
    begin
...
function TPascalCoinKeyTools.GetECIESPascalCoinCompatibilityEngine()
...
function TPascalCoinKeyTools.ComputeECIESPascalCoinEncrypt(const APublicKey
  : IECPublicKeyParameters; const APayloadToEncrypt: TBytes): TBytes;
var
...
function TPascalCoinKeyTools.ComputeECIESPascalCoinDecrypt(const APrivateKey
  : IECPrivateKeyParameters; const APayloadToDecrypt: TBytes;
  out ADecryptedPayload: TBytes): boolean;
var
...
function TPascalCoinKeyTools.RecreatePublicKeyFromAffineXandAffineYCoord
  (AKeyType: TKeyType; const AAffineX, AAffineY: TBytes)
...
function TPascalCoinKeyTools.RecreatePrivateKeyFromByteArray(AKeyType: TKeyType;
  const APrivateKey: TBytes): IECPrivateKeyParameters;
var
  domain: IECDomainParameters;
  lCurve: IX9ECParameters;
...
function TPascalCoinKeyTools.RecreatePrivateKeyFromHexStr(AKeyType: TKeyType;
  const APrivateKey: string): IECPrivateKeyParameters;
var
  lPrivate: TBytes;
  LDomain: IECDomainParameters;
...
function TPascalCoinKeyTools.EncryptPascalCoinPrivateKey
  (const APascalCoinPrivateKey, APassword: string): TRawBytes;
var
...
function TPascalCoinKeyTools.EncryptPayloadWithPassword(const APassword,
  APayload: string): string;
var
...
begin
  ...
  PasswordBytes := TConverters.ConvertStringToBytes(APassword, TEncoding.UTF8);
  PayloadToEncryptBytes := TConverters.ConvertStringToBytes(APayload,
    TEncoding.UTF8);
  EncryptedPayloadBytes := ComputeAES256_CBC_PKCS7PADDING_PascalCoinEncrypt
    (PayloadToEncryptBytes, PasswordBytes);
  Result := ComputeBase16EncodeUpper(EncryptedPayloadBytes);

end;

function TPascalCoinKeyTools.EncryptPayloadWithPublicKey(const AKeyType
  : TKeyType; const ABase58PublicKey, APayload: string): string;
var
  ...
  PayloadToEncryptBytes, EncryptedPayloadBytes: TBytes;
begin

  { TODO : This can be a lot more efficient if using the senders key as we obviously have the X and Y readily available }
  if (not ValidatePascalCoinBase58PublicKeyChecksum(ABase58PublicKey)) then
  begin
    raise Exception.Create('Invalid Base 58 Public Key');
  end;

  LPascalCoinPublicKey := ComputeBase16EncodeUpper
    (ComputeBase58Decode(ABase58PublicKey));

  if (not ValidatePublicKeyHeader(AKeyType,
    ComputeBase16Decode(LPascalCoinPublicKey), ErrorMessage)) then
  begin
    raise Exception.Create('Unable to validate public key: ' + ErrorMessage);
  end;

  if not ExtractAffineXFromPascalCoinPublicKey(LPascalCoinPublicKey,
    AffineXCoord) then
  begin
    raise Exception.Create('Unable to validate public key: X-Coordinate')
  end;

  if not ExtractAffineYFromPascalCoinPublicKey(LPascalCoinPublicKey,
    AffineYCoord) then
  begin
    raise Exception.Create('Unable to validate public key: Y-Coordinate')
  end;

  RecreatedPublicKey := RecreatePublicKeyFromAffineXandAffineYCoord(AKeyType,
    AffineXCoord, AffineYCoord);

  EncryptedPayload := ComputeECIESPascalCoinEncrypt(RecreatedPublicKey,
    PayloadToEncrypt);
...
function TPascalCoinKeyTools.DecryptPascalCoinPrivateKey
  (const AEncryptedPascalCoinPrivateKey, APassword: string;
...
begin
  Result := System.Copy(DecryptedPascalCoinPrivateKey, 4,
    System.Length(DecryptedPascalCoinPrivateKey) - 4);
end;

function TPascalCoinKeyTools.ExtractAffineXFromPascalCoinPublicKey
  (const APascalCoinPublicKey: string; out AAffineXBytes: TBytes): boolean;
var
  AffineXLength: Int32;
...
function TPascalCoinKeyTools.ExtractAffineYFromPascalCoinPublicKey
  (const APascalCoinPublicKey: string; out AAffineYBytes: TBytes): boolean;
var
...
begin
  ...
  AAffineYBytes := System.Copy(LPascalCoinPublicKeyBytes, Offset + 2,
    AffineYLength);
...
function TPascalCoinKeyTools.EVP_GetKeyIV(const APasswordBytes,
...
begin
  ...
  System.SetLength(Buf, System.Length(APlainTextBytes) + LBlockSize +
  ...
  Count := cipher.ProcessBytes(APlainTextBytes, 0,
    System.Length(APlainTextBytes), Buf, LBufStart);
...
begin
  try
    ...
    if (System.Length(ACipherTextBytes) >= SALT_MAGIC_LEN) and
      (TArrayUtils.AreEqual(Chopped,
      TConverters.ConvertStringToBytes(SALT_MAGIC, TEncoding.UTF8))) then
    begin
    ...
    Count := cipher.ProcessBytes(ACipherTextBytes, LSrcStart,
      System.Length(ACipherTextBytes) - LSrcStart, Buf, LBufStart);
    System.Inc(LBufStart, Count);
    Count := cipher.DoFinal(Buf, LBufStart);
    System.Inc(LBufStart, Count);
...
function TPascalCoinKeyTools.GetPascalCoinPublicKeyAsHexString
  (AKeyType: TKeyType; const AXInput, AYInput: TRawBytes): string;
var
  PartX, PartY, TotalPart: TBytes;
begin
  PartX := TArrayUtils.Concatenate(GetAffineXPrefix(AKeyType, AXInput),
    AXInput);
...
function TPascalCoinKeyTools.GetPascalCoinPublicKeyAsBase58(const AHexPublicKey
  : string): String;
var
  PreBase58PublicKeyBytes, PascalCoinPublicKeyBytes,
    Base58PublicKeyBytes: TBytes;
begin
  PascalCoinPublicKeyBytes := ComputeBase16Decode(AHexPublicKey);
  Base58PublicKeyBytes := ComputeBase16Decode(B58_PUBKEY_PREFIX);
  PreBase58PublicKeyBytes := TArrayUtils.Concatenate
    (TArrayUtils.Concatenate(Base58PublicKeyBytes, PascalCoinPublicKeyBytes),
    System.Copy(ComputeSHA2_256_ToBytes(PascalCoinPublicKeyBytes), 0, 4));
  Result := ComputeBase58Encode(PreBase58PublicKeyBytes);
end;

function TPascalCoinKeyTools.GetPascalCoinPrivateKeyAsHexString
  (AKeyType: TKeyType; const AInput: TRawBytes): string;
begin
  Result := ComputeBase16EncodeUpper
    (TArrayUtils.Concatenate(GetPrivateKeyPrefix(AKeyType, AInput), AInput));
end;

function TPascalCoinKeyTools.GetPascalCoinPrivateKeyEncryptedAsHexString
  (const APascalCoinPrivateKey, APassword: string): string;
var
...
begin
  ...
  PasswordBytes := TConverters.ConvertStringToBytes(APassword, TEncoding.UTF8);
  Result := ComputeBase16EncodeUpper
    (ComputeAES256_CBC_PKCS7PADDING_PascalCoinEncrypt(PascalCoinPrivateKeyBytes,
    PasswordBytes));
end;

function TPascalCoinKeyTools.GenerateECKeyPair(AKeyType: TKeyType)
...
procedure TPascalCoinKeyTools.GenerateKeyPairAndLog(AKeyType: TKeyType;
  const APassword: string; var Logger: TStringList);
var
...
begin
  ...
  Logger.Append(Format('Selected Key Type is %s',
    [GetEnumName(TypeInfo(TKeyType), Ord(AKeyType))]));
  LKeyPair := GenerateECKeyPair(AKeyType);
  Logger.Append(Format('%s ECKeyPair Generated Successfully.',
    [GetEnumName(TypeInfo(TKeyType), Ord(AKeyType))]));

  LPrivateKey := GetPrivateKey(LKeyPair);
  Logger.Append(Format('Private Key is %s',
    [ComputeBase16EncodeUpper(LPrivateKey)]));
  LPublicKeyXCoord := GetPublicKeyAffineX(LKeyPair);
  Logger.Append(Format('Public Key XCoord is %s',
    [ComputeBase16EncodeUpper(LPublicKeyXCoord)]));
  LPublicKeyYCoord := GetPublicKeyAffineY(LKeyPair);
  Logger.Append(Format('Public Key YCoord is %s',
    [ComputeBase16EncodeUpper(LPublicKeyYCoord)]));

  LPascalCoinPrivateKey := GetPascalCoinPrivateKeyAsHexString(AKeyType,
    LPrivateKey);
  Logger.Append(Format('PascalCoin Private Key is %s',
    [LPascalCoinPrivateKey]));

  Logger.Append(Format('PascalCoin Private Key Encryption Password is %s',
    [APassword]));
  LPascalCoinPrivateKeyEncrypted := GetPascalCoinPrivateKeyEncryptedAsHexString
    (LPascalCoinPrivateKey, APassword);
  Logger.Append('PascalCoin Private Key Encrypted Successfully.');
  Logger.Append(Format('PascalCoin Encrypted Private Key In Hex is %s',
    [LPascalCoinPrivateKeyEncrypted]));
...
procedure TPascalCoinKeyTools.TestEncryptPascalCoinPrivateKey
  (AKeyType: TKeyType; const APrivateKeyToEncrypt, APassword: string;
  var Logger: TStringList);
var
...
begin
  ...
  begin
    Logger.Append(Format('"%s" is an Invalid HexString',
      [APrivateKeyToEncrypt]));
    Logger.Append
      ('================================================== END ===================================================');
    Exit;
  end;

  LPascalCoinPrivateKey := ComputeBase16EncodeUpper
    (TArrayUtils.Concatenate(GetPrivateKeyPrefix(AKeyType,
    ComputeBase16Decode(APrivateKeyToEncrypt)),
    ComputeBase16Decode(APrivateKeyToEncrypt)));
  Logger.Append(Format('PascalCoin Private Key is %s',
    [LPascalCoinPrivateKey]));
  Logger.Append(Format('Password To Use For Encryption is %s', [APassword]));

  EncryptedCipherText := EncryptPascalCoinPrivateKey(LPascalCoinPrivateKey,
    APassword);

  LPascalCoinPrivateKeyEncrypted := ComputeBase16EncodeUpper
    (EncryptedCipherText);
  Logger.Append('PascalCoin Private Key Encrypted Successfully.');
  Logger.Append(Format('Encrypted PascalCoin Private Key is %s',
    [LPascalCoinPrivateKeyEncrypted]));
...
procedure TPascalCoinKeyTools.TestDecryptPascalCoinPrivateKey
  (const AEncryptedPascalCoinPrivateKey, APassword: string;
  var Logger: TStringList);
var
...
begin
  ...
  Logger.Append(Format('PascalCoin Encrypted Private Key is %s',
    [AEncryptedPascalCoinPrivateKey]));
  ...
  DecryptedSuccessfully := DecryptPascalCoinPrivateKey
    (AEncryptedPascalCoinPrivateKey, APassword, DecryptedCipherText);
  if DecryptedSuccessfully then
  begin
    ...
    LPrivateKey := ComputeBase16EncodeUpper
      (ExtractPrivateKeyFromDecryptedPascalCoinPrivateKey(DecryptedCipherText));
...
procedure TPascalCoinKeyTools.TestEncryptPascalCoinECIESPayload
  (AKeyType: TKeyType; const APascalCoinBase58PublicKey, APayloadToEncrypt
  : string; var Logger: TStringList);
var
  LPascalCoinPublicKey, ErrorMessage: string;
  AffineXCoord, AffineYCoord, PayloadToEncrypt, EncryptedPayload: TBytes;
...
begin
  ...
  Logger.Append(Format('PascalCoin Base58 Public Key is %s',
    [APascalCoinBase58PublicKey]));

  if (not ValidatePascalCoinBase58PublicKeyChecksum(APascalCoinBase58PublicKey))
  then
  begin
  ...
  if (not ValidatePublicKeyHeader(AKeyType,
    ComputeBase16Decode(LPascalCoinPublicKey), ErrorMessage)) then
  begin
    Logger.Append(Format('Error Occured While Encrypting Payload. [%s]',
      [ErrorMessage]));
    Logger.Append
      ('================================================== END ===================================================');
    Exit;
  ...
  PayloadToEncrypt := TConverters.ConvertStringToBytes(APayloadToEncrypt,
    TEncoding.UTF8);
  ...
  Logger.Append(Format('PascalCoin Public Key AffineX Coord is %s',
    [ComputeBase16EncodeUpper(AffineXCoord)]));
  ...
  Logger.Append(Format('PascalCoin Public Key AffineY Coord is %s',
    [ComputeBase16EncodeUpper(AffineYCoord)]));

  RecreatedPublicKey := RecreatePublicKeyFromAffineXandAffineYCoord(AKeyType,
    AffineXCoord, AffineYCoord);

  Logger.Append
    ('Public Key Recreated From PascalCoin Public Key AffineX and AffineY Coords.');

  EncryptedPayload := ComputeECIESPascalCoinEncrypt(RecreatedPublicKey,
    PayloadToEncrypt);

  Logger.Append('Payload Encrypted Successfully.');
  Logger.Append(Format('Encrypted Payload is %s',
    [ComputeBase16EncodeUpper(EncryptedPayload)]));

  Logger.Append
    ('================================================== END ===================================================');
end;

procedure TPascalCoinKeyTools.TestDecryptPascalCoinECIESPayload
  (AKeyType: TKeyType; const AEncryptedPascalCoinPrivateKey,
  APrivateKeyPassword, APayloadToDecrypt: string; var Logger: TStringList);
var
...
begin
  ...
  DecryptedSuccessfully := DecryptPascalCoinPrivateKey
    (AEncryptedPascalCoinPrivateKey, APrivateKeyPassword, PascalCoinPrivateKey);

  if DecryptedSuccessfully then
  begin
    Logger.Append('PascalCoin Private Key Decrypted Successfully.');
    Logger.Append(Format('PascalCoin Private Key is "%s"',
      [ComputeBase16EncodeUpper(PascalCoinPrivateKey)]));
  end
  else
  begin
    Logger.Append('An Error Occurred While Decrypting PascalCoin Private Key.');
  ...
  if (not ValidatePrivateKeyHeader(AKeyType, PascalCoinPrivateKey, ErrorMessage))
  then
  begin
    Logger.Append(Format('Error Occured While Decrypting Payload. [%s]',
      [ErrorMessage]));
    Logger.Append
      ('================================================== END ===================================================');
    Exit;
  ...
  RecreatedPrivateKey := RecreatePrivateKeyFromByteArray(AKeyType,
    ExtractPrivateKeyFromDecryptedPascalCoinPrivateKey(PascalCoinPrivateKey));
  ...
  if ComputeECIESPascalCoinDecrypt(RecreatedPrivateKey, PayloadToDecrypt,
    DecryptedPayload) then
  begin
    Logger.Append('Payload Decrypted Successfully.');
    Logger.Append(Format('Decrypted Payload is "%s"',
      [TConverters.ConverTBytesToString(DecryptedPayload, TEncoding.UTF8)]));
  end
  else
...
procedure TPascalCoinKeyTools.TestEncryptPascalCoinAESPayload(const APassword,
  APayloadToEncrypt: string; var Logger: TStringList);
var
  PasswordBytes, PayloadToEncryptBytes, EncryptedPayloadBytes: TBytes;
begin
  if not Assigned(Logger) then
  ...
  PayloadToEncryptBytes := TConverters.ConvertStringToBytes(APayloadToEncrypt,
    TEncoding.UTF8);
  ...
  Logger.Append(Format('Encrypted Payload is %s',
    [ComputeBase16EncodeUpper(EncryptedPayloadBytes)]));

  Logger.Append
    ('================================================== END ===================================================');
end;

procedure TPascalCoinKeyTools.TestDecryptPascalCoinAESPayload(const APassword,
  APayloadToDecrypt: string; var Logger: TStringList);
var
...
begin
  ...
  DecryptedSuccessfully := ComputeAES256_CBC_PKCS7PADDING_PascalCoinDecrypt
    (PayloadToDecrypTBytes, PasswordBytes, DecryptedPayloadBytes);

  if DecryptedSuccessfully then
  begin
    Logger.Append('Payload Decrypted Successfully.');
    Logger.Append(Format('Decrypted Payload is "%s"',
      [TConverters.ConverTBytesToString(DecryptedPayloadBytes,
      TEncoding.UTF8)]));
  end
  else
...
procedure TPascalCoinKeyTools.Generate_Recreate_Sign_Verify_ECDSA_Stress_Test
  (AKeyType: TKeyType; const AMessage: string; AIterationCount: Int32;
  var Logger: TStringList);
var
  LKeyPair: IAsymmetricCipherKeyPair;
  LPrivateKey, LPublicKeyXCoord, LPublicKeyYCoord, LMessage: TBytes;
  LPrivateKeyBigInteger, LPublicKeyXCoordBigInteger, LPublicKeyYCoordBigInteger
    : TBigInteger;
...
begin
  ...
  Logger.Append
    ('Please Be Patient. This Operation May Take a While For Large Iteration Count.');
  ...
  begin
    ...
    Logger.Append(Format('Private Key is %s and Sign is %s',
      [ComputeBase16EncodeUpper(LPrivateKeyBigInteger.ToByteArray()),
      GetSignAsString(LPrivateKeyBigInteger.SignValue)]));
    Logger.Append(Format('Public Key AffineX Coord is %s and Sign is %s',
      [ComputeBase16EncodeUpper(LPublicKeyXCoordBigInteger.ToByteArray()),
      GetSignAsString(LPublicKeyXCoordBigInteger.SignValue)]));
    Logger.Append(Format('Public Key AffineY Coord is %s and Sign is %s',
      [ComputeBase16EncodeUpper(LPublicKeyYCoordBigInteger.ToByteArray()),
      GetSignAsString(LPublicKeyYCoordBigInteger.SignValue)]));

    PrivateKeyRecreated := RecreatePrivateKeyFromByteArray(AKeyType,
      LPrivateKey);

    Logger.Append('Private Key Recreated.');

    PublicKeyRecreated := RecreatePublicKeyFromAffineXandAffineYCoord(AKeyType,
      LPublicKeyXCoord, LPublicKeyYCoord);
    ...
    Logger.Append(Format('R Sig is %s',
      [ComputeBase16EncodeUpper(LSig[0].ToByteArray())]));

    Logger.Append(Format('S Sig is %s',
      [ComputeBase16EncodeUpper(LSig[1].ToByteArray())]));
  ...
  Logger.Append(Format('Passed %d out of %d Tests',
    [PassedCount, AIterationCount]));
